...
主体预订闭环、库存事务回滚、订单隐私查询、入住日期边界和并发抢最后库存均通过；发现 1 个核心业务缺陷：重复办理入住未按需求返回冲突。经范围确认，员工写接口 CSRF 校验本期不实现，不计为缺陷。
...
```bash
/tmp/hotel-maven-3.9.11/bin/mvn -Dmaven.repo.local=/tmp/hotel-m2 test
```
...
- 失败：1
- 错误：0
- 失败用例：`HotelAcceptanceTest.checkInSucceedsOnceAndRepeatIsRejected`
...
- 实际结果：第二次调用未抛出冲突，仍按成功路径返回 `CHECKED_IN` 并再次更新办理时间。
- 相关文件：
...
原因证据：COLA 状态机收到 `CHECKED_IN + CHECK_IN` 时返回原状态 `CHECKED_IN`；服务代码只判断返回状态是否等于 `CHECKED_IN`，无法区分合法迁移和未发生迁移。

## 5. 风险与建议

- BUG-001 违反 FR-06、BR-04、BR-05 以及重复入住验收标准，修复后必须重跑完整后端测试。
